...
- [ ] v5/02. Fix alignement : la bordure du bas de la colonne "Projets" (sidebar) et celle de la Toolbar doivent être sur la même ligne horizontale. Auditer les hauteurs des deux éléments et aligner précisément (même `height` ou variable CSS partagée).
- [ ] v5/03. Bugfix plage du Gantt : actuellement si `timelineStart`/`timelineEnd` sont fixées (ex. 13/06 au 20/06), la zone de drag affiche une plage bien plus large (ex. jusqu'en août) car `resolveTimelineBounds()` applique un `minDays` par zoom qui écrase la plage manuelle. Corriger pour que la plage manuelle soit toujours respectée et prioritaire sur le minimum de zoom. Plafonner la plage totale à 52 semaines (1 an) maximum et 1 semaine minimum. Si l'utilisateur essaie de dépasser, bloquer avec un message clair.
- [ ] v5/04. Bouton "+ Nouvelle tâche" : le sortir de sa position actuelle (en dessous de tout, looks détaché) et l'intégrer directement DANS la liste des tâches, juste après la dernière tâche, comme s'il était lui-même une ligne de la liste (même hauteur de ligne, style discret).
- [ ] v5/05. Bouton "+ Nouveau projet" : même traitement, intégré directement dans la liste des projets de la sidebar, juste après le dernier projet.
- [ ] v5/06. Fix résidu : un bouton "Nouvelle tâche" est encore visible au-dessus de la Toolbar par erreur. Le supprimer. Auditer tout le code pour s'assurer qu'il n'existe plus qu'un seul point de création de tâche (celui intégré en v5/04).
- [ ] v5/07. Liste des tâches : gérer le texte trop long. Si le nom d'une tâche dépasse la largeur visible de la colonne, le texte passe à la ligne (`white-space: normal; word-break: break-word`) plutôt que d'être tronqué ou de débordement. La colonne des tâches doit aussi être redimensionnable en largeur (resize horizontal, comme la sidebar en v2/03), indépendamment de la largeur de la sidebar elle-même.
- [ ] v5/08. Fond opaque de la liste des tâches : actuellement le fond est transparent et on voit le calendrier bouger derrière pendant le scroll/drag. Mettre un fond opaque cohérent avec le thème actif (light/dark) sur toute la colonne des tâches.
- [ ] v5/09. Zoom en dropdown : remplacer les boutons de zoom actuels (jour/semaine/mois) par un seul bouton "Zoom" avec un menu déroulant listant les trois options. Même comportement, juste l'UI qui change (cohérent avec le pattern déjà utilisé pour l'export en v2/06).
- [ ] v5/10. Thèmes prédéfinis : ajouter 4 thèmes sélectionnables dans `themeStore.ts` : White (fond `#FFFFFF` pur), Light (fond `#F8F7F4`, actuel), Dark (actuel), Black (fond quasi noir `#0A0A0A`). Diviser le panneau de préférences existant en 3 sections distinctes : (1) Couleurs des statuts de base, (2) Statuts personnalisés, (3) Thème de l'application (sélecteur des 4 thèmes). Le thème choisi est persisté dans localStorage et s'applique immédiatement à toute l'app.
