...
export default function GanttHeader({ config, zoom, onDayWidthChange }: Props) {
  ...
  return (
    <div className="flex-none select-none" style={{ width: config.totalDays * config.dayWidth }}>
      ...
      <div
        className="flex border-b border-[var(--border)] dark:border-[var(--border)]"
        style={{ height: HEADER_WEEK_H }}
      >
        {row1.map((cell) => (
          <div
            key={cell.key}
            className="flex-none flex items-center px-2 text-xs font-medium text-neutral-500 dark:text-neutral-400 border-r border-[var(--border)] dark:border-[var(--border)] overflow-hidden"
        ...
        {row2.map((cell) => (
          <div
            ...
            className={[
              ...
              'border-r border-[var(--border)] dark:border-[var(--border)]',
              cell.isWeekend
                ? 'text-neutral-400 dark:text-neutral-600 bg-[var(--bg-subtle)] dark:bg-neutral-900'
